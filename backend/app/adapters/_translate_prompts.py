...
- items 数量必须与输入条数完全一致，index 必须与输入 index 对应；顺序无所谓，调用方会排序。
- dst 中只放中文译文本身，不要解释、不要前后缀、不要引号、不要编号。
"""
...
{{
  "items": [
    {{"index": <int>, "dst": "<English translation>"}}
  ]
}}
items count MUST equal the input count; "index" MUST match the input index. Order does not matter, the caller sorts. dst contains only the English translation, no quotes, labels, or extra whitespace.
"""

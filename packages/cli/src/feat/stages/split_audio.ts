...
export async function stageSplitAudio(taskId: string, sessionPath: string) {
  ...
  const vocalsFile = resolve(sessionPath, 'media', 'audio_vocals.wav');
  ...
  const asrData = JSON.parse(readFileSync(join(sessionPath, 'metadata', 'asr.json'), 'utf-8'));
  const totalMs = asrData.audio_info?.duration || 0;

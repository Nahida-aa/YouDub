...
io.on("connection", async (socket) => {
  ...
  socket.on('test:event', (data) => {
    ...
    io.emit('broadcast:event', { message: `Client ${socket.id} says hello to everyone!` });
  });
  ...
  socket.on("ml:voxcpm:prepare", async (data, callback) => {
    if (voxcpmPrepareTask.status === "processing") {
      return callback({ status: "error", message: "A preparation task is already running." });
    }

    const currentStatus = await checkVoxCPMStatus();
    if (currentStatus.isReady) {
      return callback({ status: "success", message: "Model is already ready." });
    }

    if (currentStatus.exists) {
      voxcpmPrepareTask.status = "processing";
      voxcpmPrepareTask.progress = { message: "Starting download...", percent: 0 };
      ...
      (async () => {
        try {
          ...
          voxcpmPrepareTask.progress = { message: "All models downloaded!", percent: 100 };
          io.emit("ml:voxcpm:progress", voxcpmPrepareTask.progress);
          ...
          voxcpmPrepareTask.progress = { message: `Download failed: ${error.message}`, percent: 0 };
          io.emit("ml:voxcpm:progress", voxcpmPrepareTask.progress);
        }
      })();
      ...
      callback({ status: "error", message: "Model weights not found. Please run Python backend first." });
    }
  });

...
async function downloadFile(url: string, dest: string, onProgress: (pct: number, msg: string) => void) {
  ...
  while (true) {
    ...
    if (totalSize > 0) {
      onProgress(Math.floor((received / totalSize) * 100), `${dest.split('/').pop()} ${Math.floor((received / totalSize) * 100)}%`);
    }
  }
  writer.end();
}

export async function downloadWhisper(onProgress: (pct: number, msg: string) => void) {
  ...
  const allFiles = [
    ...ONNX_FILES.map(f => ({ url: `${HF_BASE}/${f}`, dest: join(outDir, f.replace('onnx/', '')) })),
    ...TOKENIZER_FILES.map(f => ({ url: `${HF_BASE}/${f}`, dest: join(tokenizerDir, f) })),
  ];
